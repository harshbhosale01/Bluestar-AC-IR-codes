...
class RawIRMessage():
  ...
  def display_binary(self, binary_str):
    ...
    max_col_row=len(ws['B'])
    for col_num, byte in enumerate(binary_list, start=1):
        cell = ws.cell(row=max_col_row+1, column=col_num+1)
        cell.value = byte
	

    # Save the workbook
    wb.save('Blue_star_data.xlsx')
    print(f"Binary string inserted into '{'Blue_star_data.xlsx'}' in sheet '{'BinaryData'}'.")
    ...
    self.output.write("\n  Bits: %d\n"
                      ...
                      (bits, ("0x{0:0%dX}" % (bits / 4)).format(num),
                       ...
                       rev_num, byte_wise_binary_str, rev_binary_str))
